...
char AtBoardPosition(const Square &square, const Board &brd)
{
    ...
    if (square & brd.white_pawn_)
        return 'P';
    else if (square & brd.white_knight_)
        return 'N';
    else if (square & brd.white_bishop_)
        return 'B';
    else if (square & brd.white_rook_)
        return 'R';
    else if (square & brd.white_queen_)
        return 'Q';
    else if (square & brd.white_king_)
        return 'K';
    else if (square & brd.black_pawn_)
        return 'p';
    else if (square & brd.black_knight_)
        return 'n';
    else if (square & brd.black_bishop_)
        return 'b';
    else if (square & brd.black_rook_)
        return 'r';
    else if (square & brd.black_queen_)
        return 'q';
    else if (square & brd.black_king_)
        return 'k';
...
std::string BoardConsoleGUI::BoardAsString(const Board &board)
{
    std::string ret = "";
    ...
    ret += std::to_string(int(idx / 8) + 1);

    while (idx >= 0)
    {
        if ((idx + 1) % 8 == 0)
        {      
            ret += " |" + std::string(1, printer.at(idx)) + " |\n";
            idx -= 15;
            if (int(idx / 8) + 1 > 0)
            {              
                ret += std::to_string(int(idx / 8) + 1);
