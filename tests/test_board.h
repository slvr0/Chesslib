...
TEST(SetFromFenCastlingState, CorrectCastlingState) {
  ...
  ASSERT_EQ(b1.getState().castle , 15);
  ASSERT_EQ(b2.getState().castle , 3);
  ASSERT_EQ(b3.getState().castle , 8);
  ASSERT_EQ(b4.getState().castle , 0);
}
...
TEST(ChessBoardInsertPiece, InsertCorrectlyPawn) {  
  ...
  b1.insertPiece(0, Chesslib::PieceType::PAWN);
