...
void Board::applyMove(const Transition& transition) {
    ...
    //debugging, we have to many allowed captures to Perfqt
    static int captures = 0;
/*     if(1ULL << dst & all()) {
        print(++captures);
    } */

    removePiece(src, charToPieceType(instr), state_.white_acts);
    removePiece(dst, pieceAt(dst).second, !state_.white_acts);
    insertPiece(dst, charToPieceType(instr), state_.white_acts);

/*     if(instr == 'o' || instr == 'O') { updateCastleStatus(src, dst, instr); }   
    else if(instr == 'E') { updateBoardEnpassaint(src, dst); }
    else {
        removePiece(src, charToPieceType(instr), state_.white_acts);
        removePiece(dst, pieceAt(dst).second, !state_.white_acts);
        insertPiece(dst, charToPieceType(instr), state_.white_acts);
...
void Board::removePiece(const int & idx, Chesslib::PieceType type, bool white) {
    ...
    //INVALID == VOID, nothing there
    if(type == PieceType::INVALID) {
        /* std::cout << "Debug << Trying to remove an INVALID piece || ";
        std::cout << "PieceType == " << static_cast<char> (type) << std::endl; */
        return;
    }
...
SpecificPiece Board::pieceAt(const int & idx) const {
    ...
    auto locateType  = [&]() {
        ...
        if(pieces_.pawns &  idx_64) return Chesslib::PieceType::PAWN;
...
std::string BoardConsoleGUI::translateTransition(const Chesslib::Board & board, const Transition & transition) {
    char charP = (char)chesspieceNotationMap.at(board.pieceAt(transition.srcdest.first));
...
/*         if((dst - src) * (dst - src) == 256) {
    ...
    //remove piece (src, pTypeChar[p], state.whiteacts) for all cases !
    else if (instr == 'b'){                    
